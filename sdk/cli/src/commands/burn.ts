...
export const command = "burn";
export const describe = "Burn tokens from your token account";

export function builder(yargs: Argv) {
  return yargs
    .option("mint", { alias: "m", type: "string", demandOption: true, description: "Mint address" })
    .option("from", { type: "string", demandOption: true, description: "Token account to burn from" })
...
export async function handler(argv: ArgumentsCamelCase) {
  ...
  const burner = loadKeypair(argv.keypair as string);
  ...
  const [rolePda] = PublicKey.findProgramAddressSync([ROLE_SEED, stablecoinPda.toBuffer(), burner.publicKey.toBuffer()], programId);

  const amountBuf = Buffer.alloc(8);
  amountBuf.writeBigUInt64LE(BigInt(argv.amount as string));
  const data = Buffer.concat([disc("burn_tokens"), amountBuf]);
  ...
  const ix = new TransactionInstruction({
    keys: [
      ...
      { pubkey: new PublicKey(argv.from as string), isSigner: false, isWritable: true },
